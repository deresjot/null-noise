...
- Mobile Home-Einstiege wurden visuell priorisiert: Titel-Suche bleibt primär, Richtungsstart bleibt sekundär, ohne neues Feature und ohne neue Produktlogik.
- Der lokale Check lief über Playwright-DOM/Screenshots, direkte mobile axe-Prüfung, Build und bestehende A11y-Tests.
- Die Ersteinschätzung hat intern jetzt eine Evidence-Normalform mit TMDb als aktiver Quelle; Does the Dog Die und Common Sense Media sind vorbereitet, bleiben ohne Flag und API-Key aber Noop/Fallback.
- Sichtbar werden daraus weiterhin nur kurze Gründe und Vorläufigkeit, keine Scores, Prozentwerte oder Rankings. DTTD-Zugriff/Nutzungsbedingungen und CSM-API/Partnerschaft/lokale Speicherung bleiben vor Aktivierung offen.
- Die Evidence-Regeln wurden mit lokalen Mock-Beispielen kalibriert: ruhige, wechselhafte, audio-/action-intensive, emotional schwere und visuell dichte Profile werden testbasiert getrennt geprüft.
