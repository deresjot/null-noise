...
- Tagesabschluss 22. August 2026: `0.8.6-domain-cleanup.20260822` bündelt die Entfernung der früheren Custom-Domain und der nicht mehr existierenden öffentlichen Projekt-Mailbox. `https://null-noise.vercel.app` ist kanonische Production-Adresse; sichtbarer Kontakt läuft ausschließlich über das Formular. Vercel-ENV und Git-Verknüpfung wurden aktualisiert, Commit `638d4e3` wurde gepusht und als Production bereitgestellt.
- Abschlussprüfung: Lint, Production-Build, sechs direkt betroffene Unit-Tests, zwei gezielte Release-/Changelog-Playwright-Tests und `git diff --check` bestanden; Live-Smoke bestätigte Startseite, Kontakt, Impressum, Datenschutz und Barrierefreiheit ohne veraltete Domain-, Mailbox- oder `mailto:`-Referenzen.
