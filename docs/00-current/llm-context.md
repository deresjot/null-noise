...
- Tagesabschluss 22. August 2026: `0.8.6-domain-cleanup.20260822` ist auf `https://null-noise.vercel.app` live. Frühere Custom-Domain- und öffentliche Projekt-Mailbox-Verweise sind aus Anwendung, Metadaten, Tests, ENV-Beispielen und kanonischer Doku entfernt; Kontakt bleibt über das Formular möglich. Vercel ist mit `deresjot/null-noise` verbunden.
- Prüfung: Lint, Production-Build, sechs gezielte Unit-Tests, zwei Release-/Changelog-Playwright-Tests und `git diff --check` bestanden; Live-Smoke auf Startseite und vier betroffenen Inhaltsseiten bestätigte korrekte Canonicals und keine veralteten Referenzen.
- aktiver lokaler Arbeitsbranch: `null-noise`
- Release-Metadaten stehen auf `0.8.6-domain-cleanup.20260822`
...
- Preview-Deploys entstehen nach Push über die GitHub-/Vercel-Verknüpfung; Production wird anschließend bewusst geprüft und promotet
...
- Mobile-Calm-Feedback-Pass 21. Juni 2026: Cards haben wieder mehr sichtbare Kartenruhe, klarere Innenabstände, größere Touch-Ziele und stärkere Trennung von Poster, Titel, erster Einschätzung, lokalem Status und Aktionen.
- Status/Feedback 21. Juni 2026: Statusmeldungen nutzen Icon, Statuskopf, Rahmenlogik und Text als redundantes Leitsystem. Die schnelle Rückmeldung zur eigenen Wahrnehmung ist als geführte Auswahlkarten im Lesefluss gestaltet; das lange Vier-Fragen-Formular bleibt sekundär.
- Produktleitbild 21. Juni 2026: null-noise ist ein ruhiges, mobile-first Bewertungsportal statt Streaming-Katalog. Motion, Icons und Farbe sind gewünscht, müssen aber Reduced Motion, Forced Colors, sichtbare Fokuszustände und Textalternativen respektieren.
...
- Aktuelle lokale Checks am 21. Juni 2026: `npm run lint` und `npm run test:a11y` bestanden. `npm run test:unit` bleibt ohne `NULL_NOISE_TEST_DATABASE_URL` blockiert. Kein Push, kein Deploy in diesem Pass.
